 ...
 * @forked from jbouny / https://github.com/jbouny
 *
 * Work based on :
...
THREE.ShaderLib['water'] = {
  ...
  vertexShader: [
    ...
    'vec2 getRadial(vec2 center)',
    '{',
    'vec2 diff = position.xy - center;',
    'vec2 D = (diff)/length(diff);',
    'return D;',
    '}',
    ...
    // Calculate new vertex position and normals
    '  uDirection = vec2(1., 0.);',
    '  vUv = uv;',
    '  float W1 = wave(uDirection, uAmplitude, uSpeed, w1);',
    '  float H = W1;',
    '  float dHdx = dWdx(uDirection, uAmplitude, uSpeed, w1);',
    '  float dHdy = dWdy(uDirection, uAmplitude, uSpeed, w1);',
    '  vec3 N_x_y = normalize(vec3(-dHdx, -dHdy, 1));',  // N(x, y); surface normal at any given point
    '  vNormal = N_x_y;',
    '  vec3 newPosition = position + normal * H;',
